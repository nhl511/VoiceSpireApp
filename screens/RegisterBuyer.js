...
const RegisterBuyer = ({ navigation }) => {
  ...
  const handleRegisterBuyer = async () => {
    ...
    if (checked) {
      try {
        await axios
          .post(registerBuyerURL, registerBuyer, { headers })
          .then((response) => {
  ...
  return (
    <SafeAreaView
      style={tw`flex-1 android:pt-15 bg-[#ffd600] justify-center items-center`}
    >
      <ScrollView showsVerticalScrollIndicator={false}>
        <View>
          <Text
            style={tw`text-transform: uppercase text-6.5 font-semibold text-center`}
          >
            Tạo tài khoản tuyển dụng
          </Text>
          <Text style={tw`text-center text-3.8`}>
        ...
        <View style={tw`mt-5`}>
          ...
          <TouchableOpacity
            style={tw` flex items-center mt-5 `}
            onPress={handleRegisterBuyer}
          >
            <Text
              style={tw`bg-[#ffffff] text-center px-12 py-2 text-5 rounded-3xl`}
            >
              Đăng ký
            </Text>
          </TouchableOpacity>
          <Pressable onPress={() => navigation.navigate("RegisterMenu")}>
            <View style={tw`mt-20 flex-row items-center gap-2 justify-center`}>
